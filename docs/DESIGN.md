...
The mark is a monogram: interlocking block-letter **"J" (neutral) + "S" (`#00ADEF` blue)**. The original source file also carried a "Jarvis Studio" wordmark below it and sat on a near-black background; the current `public/logo.svg` is a clean redraw of the monogram alone, on transparency.

- **File status — resolved.** `public/logo.svg` is now a **clean vector redraw**: three polygons, transparent background, 1.4KB. It replaces the auto-traced conversion of the original JPEG (~240 near-duplicate path fragments reconstructing the shapes *and* a baked-in black rectangle, ~250KB) that this section previously described. The redraw is monogram only; the wordmark is set in Clash Display as real text at the call site, which is why the nav and footer read correctly at any size.
- **Light mode — resolved, and the mechanism matters.** The mark is **inlined** by `components/ui/Logo.tsx` with the "J" on `currentColor`, so it takes `--text-primary` and follows the theme; the "S" stays literal `#00ADEF`. Inlining is not a preference here — an SVG referenced through `<img>` is an isolated document, so `currentColor` inside it resolves against the SVG's own root rather than the page, and a themed mark is unreachable that way. The standalone `public/logo.svg` keeps a white "J", which is the correct on-dark form for a brand asset consumed anywhere else.
- **The blue is literal, not `--accent`.** `--accent` darkens to brand-800 in light mode to clear WCAG contrast for text and UI borders. A logo is neither, and shifting the mark's blue between themes would break its recognisability for a rule that does not apply to it.
- **Retired with the redraw:** the fixed-size black rounded container and the `object-cover` crop that went with it. Both existed only to make an opaque square asset survive a light background; the container cropped a 4:3 mark into a square.
...
| **Alternating feature blocks** | ~~Services page (per service line)~~, About page (differentiators) | **Superseded for /services, twice.** The page's DIRECTION CONTRACT refused this arrangement as the category default that only serves a visitor who already knows what they need; the six lines are now a comparison grid (MOTION_REDESIGN §5.9). Still live for the About page. |
...
- **A six-up comparison grid**, one card per service line (Web Dev, App Dev, SaaS, CRM, AI Automation, Marketing & Design), each with its authored graphic and three concrete capabilities listed (not vague marketing copy — per PRD user story: prospective clients want to immediately tell if the studio does what they need). A diagnostic above it promotes the services matching the visitor's situation to full-width rows; nothing is ever hidden. See MOTION_REDESIGN §5.9 and the DIRECTION CONTRACT in `app/services/page.tsx`.
  - *This line previously read "5 alternating feature blocks" and was stale on both counts — there have been six services since before that arrangement was refused.*
...
- ~~**Logo needs a proper redraw, not just a format fix:**~~ **Closed.** A clean Illustrator redraw of the monogram was supplied and is now `public/logo.svg` — three polygons, transparent, 1.4KB, down from ~250KB of auto-traced fragments. Both workarounds this raised (the baked-in black background and the rounded container hiding it) are gone, and the light-mode variant it asked for is the same file inlined with the "J" on `currentColor`. See §2.1. One thing did **not** come with it: the wordmark was not redrawn, only the monogram. That is sufficient today because every current usage sets the wordmark as live text, but a lockup for favicons, OG images or print would still need one.
